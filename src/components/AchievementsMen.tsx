...
import { Trophy, Plus, Trash2, Search, Award, Star, Flame } from 'lucide-react';
...
export const AchievementsMen: React.FC<AchievementsMenProps> = ({
  ...
  const [motyCount, setMotyCount] = useState(0);
  const [hallOfFame, setHallOfFame] = useState(false);
  const [streakWins, setStreakWins] = useState(0);

  const maleSuperstars = superstars.filter((s) => s.tier !== 'Female');
  ...
  return (
    <div className="max-w-[1920px] mx-auto p-4 md:p-6 space-y-6 text-slate-100">
      {/* Header Banner */}
      <div className="p-6 bg-gradient-to-r from-amber-950/80 via-slate-900 to-amber-950/80 border border-amber-500/40 rounded-xl shadow-2xl flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-amber-500/20 border border-amber-500/40 rounded-xl shadow-lg">
            <Trophy className="w-7 h-7 text-amber-400" />
          </div>
          <div>
            <h1 className="text-2xl font-black uppercase text-amber-300 tracking-wider">
              Men's Division Achievements & Accolades
            </h1>
            <p className="text-xs text-slate-300 mt-0.5">
              Track Royal Rumble wins, Money in the Bank briefcases, Elimination Chamber victories, Grand Slam status, and Hall of Fame status.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-5 bg-slate-900 border border-slate-800 rounded-xl shadow-lg space-y-4">
          ...
          <form onSubmit={handleCreate} className="space-y-3 text-xs">
            <div>
              ...
              <input
                type="text"
                list="male-superstars-list"
                placeholder="Type or select superstar..."
                value={superstarName}
                onChange={(e) => setSuperstarName(e.target.value)}
                className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-white focus:outline-none focus:border-amber-500"
              />
              <datalist id="male-superstars-list">
                {maleSuperstars.map((s) => (
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="block text-slate-400 mb-1">Win Streak</label>
                <input
                  type="number"
                  min="0"
                  value={streakWins}
                  onChange={(e) => setStreakWins(Number(e.target.value))}
                  className="w-full p-2 bg-slate-950 border border-slate-700 rounded text-white"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <label className="flex items-center gap-2 bg-slate-950 p-2 border border-slate-800 rounded cursor-pointer">
                ...
                />
                <span className="font-semibold text-amber-300">Grand Slam</span>
              </label>
              ...
              <label className="flex items-center gap-2 bg-slate-950 p-2 border border-slate-800 rounded cursor-pointer">
                <input
                  ...
                  className="accent-amber-500"
                />
                <span className="font-semibold text-yellow-400">Hall of Fame</span>
              </label>
            </div>

            <button
              type="submit"
              className="w-full py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-lg transition shadow-lg mt-2 uppercase tracking-wide"
            >
              Save Achievement
            </button>
          </form>
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="overflow-x-auto bg-slate-900 border border-slate-800 rounded-xl shadow-lg">
            <table className="w-full text-xs text-left text-slate-200">
              <thead className="bg-slate-950 uppercase font-bold text-[10px] text-amber-400 border-b border-slate-800">
                <tr>
                ...
                {filteredList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-6 text-center text-slate-500 italic">
                      No achievement records found. Use the form on the left to add one!
                    </td>
                  </tr>
                ) : (
                  filteredList.map((a) => (
                    <tr key={a.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="p-3">
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          {a.hallOfFame && (
                            <span className="px-1.5 py-0.5 bg-yellow-500/20 text-yellow-300 border border-yellow-500/40 rounded text-[10px] font-bold">
                              Hall of Fame
                            </span>
                          )}
                          {a.streakWins > 0 && (
                            <span className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 rounded text-[10px] font-bold">
                              {a.streakWins} W Streak
                            </span>
                          )}
                        </div>
                      </td>
